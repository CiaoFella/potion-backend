...
export const storedReportsController = {
  ...
  async requestReport(req: any, res: any) {
    try {
      ...
      // Calculate period based on duration (matching transactions page)
      if (duration) {
        switch (duration.toLowerCase()) {
          ...
          case 'last2years':
            ...
            break;
          // Legacy support for existing duration types
          case 'quarterly':
            start = new Date();
        ...
        return res.status(400).json({
          error: 'Either duration or start/end dates must be provided',
        });
      }
  ...
  async listReports(req: any, res: any) {
    try {
      ...
      let queryChain = Report.find(query).sort({ createdAt: -1 });
